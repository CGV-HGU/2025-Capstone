...
system::system(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    : slam_(slam),
      node_(node),
      custom_qos_(rmw_qos_profile_sensor_data),
      ...
      tracking_status_pub_(node_->create_publisher<std_msgs::msg::Bool>("~/tracking_status", 1)),
...
void system::publish_tracking_status(bool tracking, const rclcpp::Time& stamp) {
    (void)stamp;
...
void system::init_pose_callback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg) {
    if (camera_optical_frame_.empty()) {
        RCLCPP_ERROR(node_->get_logger(), "Camera link is not set: no images were received yet");
        return;
    }
    Eigen::Translation3d trans(msg->pose.pose.position.x, msg->pose.pose.position.y, msg->pose.pose.position.z);
    Eigen::Quaterniond rot_q(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z);
    ...
    try {
        auto map_to_initialpose_frame = tf_->lookupTransform(
            ...
            tf2::durationFromSec(0.0));
        map_to_initialpose_frame_affine = tf2::transformToEigen(map_to_initialpose_frame.transform);
    } catch (tf2::TransformException& ex) {
        RCLCPP_ERROR_STREAM_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000, "Transform failed: " << ex.what());
        return;
    }
    ...
    Eigen::Matrix4d cam_pose_cv = (rot_cv_to_ros_map_frame * map_to_initialpose_frame_affine * initialpose_affine * robot_base_frame_to_camera_affine).matrix();
...
void mono::callback(sensor_msgs::msg::Image::UniquePtr msg_unique_ptr) {
    ...
    publish_tracking_status(cam_pose_wc != nullptr, msg->header.stamp);
...
void stereo::callback(const sensor_msgs::msg::Image::ConstSharedPtr& left, const sensor_msgs::msg::Image::ConstSharedPtr& right) {
    ...
    publish_tracking_status(cam_pose_wc != nullptr, left->header.stamp);
    if (publish_keyframes_) {
...
void rgbd::callback(const sensor_msgs::msg::Image::ConstSharedPtr& color, const sensor_msgs::msg::Image::ConstSharedPtr& depth) {
    ...
    publish_tracking_status(cam_pose_wc != nullptr, color->header.stamp);
    if (publish_keyframes_) {
