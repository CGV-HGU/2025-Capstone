...
public:
    ...
    void setParams();
    void publish_tracking_status(bool tracking, const rclcpp::Time& stamp);
    ...
    std::shared_ptr<rclcpp::Publisher<std_msgs::msg::Bool>> tracking_status_pub_;
...
public:
    ...
    ModifiedSubscriber(typename message_filters::Subscriber<M>::NodePtr node, const std::string& topic, const rmw_qos_profile_t qos, rclcpp::SubscriptionOptions options)
        : message_filters::Subscriber<M>(node, topic, qos, options) {
    }
    ModifiedSubscriber(NodeType* node, const std::string& topic, const rmw_qos_profile_t qos, rclcpp::SubscriptionOptions options)
        : message_filters::Subscriber<M>(node, topic, qos, options) {
    }
