...
class OMOR1MiniNode(Node):
  def __init__(self):
    ...
    self.tf_only_mode = False # TF 브로드캐스트 전용 모드 여부
    ...
    # Services
    self.srvSetTFOnlyMode = self.create_service(Onoff, 'set_tf_only_mode', self.cbSrv_setTFOnlyMode)
  ...
  def cbSrv_setTFOnlyMode(self, request, response):
    self.tf_only_mode = request.set
    print(f"TF-only mode set to {'enabled' if self.tf_only_mode else 'disabled'}")
    return response
  ...
  def update_odometry(self, odo_l, odo_r, trans_vel, orient_vel, vel_z):
    ...
    if not self.tf_only_mode:  # tf 브로드캐스트 전용 모드가 아니면 정상 업데이트
      if self.use_gyro:
    ...
    odom.pose.pose.position.x = self.odom_pose.x if not self.tf_only_mode else 0.0
    odom.pose.pose.position.y = self.odom_pose.y if not self.tf_only_mode else 0.0
